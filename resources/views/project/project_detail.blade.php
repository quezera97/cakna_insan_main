...
@section('content')
                        ...
                        <div class="sm:pl-8 sm:py-8 mt-2 pt-4 sm:mt-0 text-center sm:text-left">
                            @auth
                                <button
                                    ...
                                    data-id="{{ $project->banner?->id }}"
                                    data-model="{{ $project->banner::class }}"
                                    data-details="{{ $project->banner?->caption }}"
                                    data-column="caption"
                                    aria-label="Edit">
                                    @include('livewire.components.svg-edit')
                                </button>
                            @endauth
                            <p class="text-lg mb-4">{!! $project->banner?->caption !!}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="lg:hidden md:hidden">
        @if (isset($project->projectable?->pastProjectImages))
            @foreach ($project->projectable->pastProjectImages->where('reference_type', $project->projectable_type) as $key => $image)
                <section class="bg-white body-font rounded-lg shadow-lg p-6 text-gray-700 border border-gray-100 lg:m-10 m-5">
                            ...
                            <div class="lg:flex-grow md:w-1/2 lg:pl-24 md:pl-16 flex flex-col md:items-start md:text-left items-center text-center">
                                ...
                                @auth
                                    <button
                                        class="w-24 open-modal bg-indigo-500 hover:bg-indigo-600 text-white font-bold p-2 rounded-full mr-2"
                                        data-id="{{ $image->id }}"
                                        data-model="{{ $image::class }}"
                                        data-details="{{ $image->caption }}"
                                        data-column="caption"
                                        aria-label="Edit">
                                        @include('livewire.components.svg-edit')
                                    </button>
        ...
        @elseif (isset($project->projectable?->incomingProjectImages))
            @foreach ($project->projectable->incomingProjectImages->where('reference_type', $project->projectable_type) as $key => $image)
                <section class="bg-white body-font rounded-lg shadow-lg p-6 text-gray-700 border border-gray-100 lg:m-10 m-5">
                    <div class="container mx-auto flex px-5 py-5 md:flex-row flex-col items-center">
                        <div class="lg:max-w-lg lg:w-full md:w-1/2 w-5/6 mb-10 md:mb-0">
                            <img class="object-cover object-center rounded h-96" alt="{{ $project->projectable?->title }}" src="{{ asset('storage/'.$image->image_path) }}">
