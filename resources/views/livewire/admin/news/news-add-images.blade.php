                        ...
                        <div class="container px-5 mx-auto">
                            <div class="mx-auto mb-5 text-center">
                                ...
                                <a wire:click="setActive('{{ $otherImages['links'] }}')" class="{{ $activeLink == $otherImages['links'] ? 'bg-gray-100 border-indigo-500 text-indigo-500' : 'border-gray-200 hover:text-gray-900' }} sm:px-6 py-3 w-1/2 sm:w-auto justify-center sm:justify-start border-b-2 title-font font-medium inline-flex items-center leading-none border-gray-200 hover:text-gray-900 tracking-wider">
                                    ...
                                    {{ __('ui_text.other_image') }}
                                </a>
                            </div>
                            <div x-data="{ typeOfImage: @entangle('activeLink') }">
                                <div x-show="typeOfImage == '{{ $mainImage['links'] }}'">
                                    <h3 class="xl:w-1/4 lg:w-1/3 md:w-1/2 w-2/3 block mx-auto text-gray-500 text-xs tracking-widest title-font mb-1">{{ ucwords($mainImage['links']).' '.__('ui_text.images') }}</h3>
                                    <form wire:submit.prevent="save('{{ $selectedNewsDetail->id }}','{{ $activeLink }}')">
                                        ...
                                        <div class="xl:w-1/4 lg:w-1/3 md:w-1/2 w-2/3 block mx-auto">
                                            ...
                                            <div wire:loading wire:target="mainImage.main_image">{{ __('ui_text.uploading') }}...</div>
                                        </div>
                                ...
                                <div x-show="typeOfImage == '{{ $bannerImage['links'] }}'">
                                    <h3 class="xl:w-1/4 lg:w-1/3 md:w-1/2 w-2/3 block mx-auto text-gray-500 text-xs tracking-widest title-font mb-1">{{ ucwords($bannerImage['links']).' '.__('ui_text.images') }}</h3>
                                    <form wire:submit.prevent="save('{{ $selectedNewsDetail->id }}','{{ $activeLink }}')">
                                        @csrf
                                        <div class="xl:w-1/4 lg:w-1/3 md:w-1/2 w-2/3 block mx-auto">
                                            ...
                                            <div wire:loading wire:target="bannerImage.banner_image">{{ __('ui_text.uploading') }}...</div>
                                        </div>
                                        <div class="p-2 xl:w-1/4 lg:w-1/3 md:w-1/2 w-2/3 block mx-auto object-cover object-center rounded mt-5">
                                ...
                                <div x-show="typeOfImage == '{{ $otherImages['links'] }}'">
                                    <h3 class="xl:w-1/4 lg:w-1/3 md:w-1/2 w-2/3 block mx-auto text-gray-500 text-xs tracking-widest title-font mb-1">{{ ucwords($otherImages['links']).' '.__('ui_text.images') }}</h3>
                                    <form wire:submit.prevent="save('{{ $selectedNewsDetail->id }}','{{ $activeLink }}')">
                                        @csrf
                                        <div class="w-1/4 mx-auto">
                                            ...
                                            <div wire:loading wire:target="otherImages.other_images">{{ __('ui_text.uploading') }}...</div>
                                        </div>
