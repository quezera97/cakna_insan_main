<div class="lg:w-1/4 md:w-1/2 p-4 w-full">
    <a class="block relative h-48 rounded overflow-hidden">
        <h3 class="text-gray-500 text-xs tracking-widest title-font mb-1">{{ ucwords($selectedNewsImage->type).' '.__('ui_text.images') }}</h3>
        <img alt="{{ $selectedNewsImage->title }}" class="object-cover object-center w-full h-full block" src="{{ asset('storage/'.$selectedNewsImage->image_path) }}">
    </a>
    <div class="mt-4">
        <h2 class="text-gray-900 title-font text-lg font-medium">{{ $selectedNewsImage->title }}</h2>
    ...
    <div class="my-5 text-center">
        ...
        <button wire:click="editSelectedImage('{{ $selectedNewsImage->id }}')" class="mx-auto text-white bg-indigo-500 border-0 py-2 px-3 focus:outline-none hover:bg-indigo-600 rounded text-sm">{{ __('ui_text.edit') }}</button>
        <button wire:click="openConfirmationModal('deleteImages', {{ $selectedNewsImage->id }})" class="mx-auto text-white bg-red-500 border-0 py-2 px-3 focus:outline-none hover:bg-red-600 rounded text-sm">{{ __('ui_text.delete') }}</button>
    </div>
        ...
        <div class="bg-gray-100 bg-opacity-75 fixed z-10 inset-0 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen">
                <div class="bg-white w-1/2 h-auto rounded shadow-lg p-8">
                    ...
                    <form wire:submit.prevent="edit('{{ $selectedNewsImage->id }}')">
                        @csrf
                        <div class="mb-5">
