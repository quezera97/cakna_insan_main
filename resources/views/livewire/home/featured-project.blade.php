                ...
                <div class="lg:flex-grow md:w-1/2 lg:pr-24 md:pr-16 flex flex-col md:items-start md:text-left mb-16 md:mb-0 items-center text-center">
                    ...
                    <div class="flex justify-center">
                        @if (isset($featuredProject[$currentProjectIndex]->donationDetail) && !is_null($featuredProject[$currentProjectIndex]->donationDetail?->donation_url))
                            <a href="{{ 'https://toyyibpay.com/'.$featuredProject[$currentProjectIndex]->donationDetail?->donation_url }}" target="__blank" class="inline-flex text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg">Infaq Now</a>
                        @endif
                        <a href="{{ route('project_detail', $featuredProject[$currentProjectIndex]) }}" class="ml-4 inline-flex text-gray-700 bg-gray-200 border-0 py-2 px-6 focus:outline-none hover:bg-gray-300 rounded text-lg">Details</a>
                    </div>
                </div>
